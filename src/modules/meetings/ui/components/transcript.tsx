    ...
    return (
        <div className="bg-white rounded-lg border px-4 py-5 flex flex-col gap-y-4 w-full">
            ...
            <div className="relative">
                <Input placeholder="Search Transcript" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-7 h-9 w-[240px]" />
                    ...
                    {filteredData.map((item) => {
                        return (
                            <div key={item.start_ts} className="flex flex-col gap-y-2 hover:bg-muted p-4 rounded-md border">
                                ...
                                <Highlighter className="text-sm text-neutral-700" highlightClassName="bg-yellow-200" searchWords={[searchQuery]} autoEscape={true} textToHighlight={item.text} />
                            </div>
                        )
                    })}
                </div>
